...
@Named(value = "cBDisbursementController")
@ViewScoped
public class CBDisbursementController implements Serializable {
    ...
    List<FmsCapitalBudgetDetail> fmsCapitalBudgetDetailsList = new ArrayList<>();
    ...
    public List<FmsCapitalBudgetDetail> getFmsCapitalBudgetDetailsList() {
        if (fmsCapitalBudgetDetailsList == null) {
            fmsCapitalBudgetDetailsList = new ArrayList<>();
        }
        return fmsCapitalBudgetDetailsList;
    }

    public void setFmsCapitalBudgetDetailsList(List<FmsCapitalBudgetDetail> fmsCapitalBudgetDetailsList) {
        this.fmsCapitalBudgetDetailsList = fmsCapitalBudgetDetailsList;
    }
    ...
    public CBDisbursementController() {
...
//..... add more information by calling fields defined in the object 
    ...
    public void onChangeJobNo(ValueChangeEvent event) {
        if (event.getNewValue() != null) {
            ...
            fmsCapitalBudget1List = fmsCapitalBudgetBeanLocal.findByCCSSJandPMSandBYRAuthorized(fmsLuBudgetYear, fmsCostcSystemJunction, pmsWorkAuthorization);
        }
    }
    ...
    public void RequestCodeChange(ValueChangeEvent event) {
        if (!event.getNewValue().toString().isEmpty()) {
            ...
            fmsCapitalBudgetDetailsList = fmsCapitalBudgetDetailBeanLocal.fetchSelectedCBRequest(fmsCapitalBudget1);
        }
    }

    public void CbGlCodeChange(ValueChangeEvent event) {
        if (!event.getNewValue().toString().isEmpty()) {
            budgetCode.setBudgetCode(event.getNewValue().toString());
            fmsCapitalBudgetDetail = fmsCapitalBudgetDetailBeanLocal.fetchCapBudgetDetail(fmsLuBudgetYear, fmsCostCenter, budgetCode);
            fmsCbDisbursementList = fmsCBDisbursementBeanLocal.fetchDisbursedCB(fmsCapitalBudgetDetail);
            recreateDisbursementModel();
        }
    }
    ...
    public void searchCBDisbByCriteria() {
        ...
        fmsCbDisbursementList = fmsCBDisbursementBeanLocal.fetchDisbursedBudget(fmsLuBudgetYear, fmsCostCenter, pmsWorkAuthorization);
