...
//.
//.KAMSI_CAL_0.1 is a javascript Program that is designed to implement Ethiopian Calendar
//.Designed and implemented by SINISHAW KASSA
//.DEC-2010
//.INSA
//****************************************************************************
var initialDay = 1, initialMonth = 1, initialYear = 1899, totalDaysForMonth, dayListValue, localId, outputGC, language = "Amh";
//*****************************************************************************
...
function showCal() {
    ...
    //toolTip = "<div id='tooltip' style='visibility:hidden; z-index:1100;border:1px solid White;font-size:12px;background-color:green;'></div>";
    ...
    monthList = "<select id='month_list' onchange='changeMonthStructure(displayToday2()),changeHeader()' ><option value=''></option> </select>";
    yearList = "<select id='year_list'  onchange='changeMonthStructure(displayToday2()),changeHeader()'><option value='' ></option></select>";
    choice = "<select id='choice_list'  onchange='setLanguage(this.id),clearMonth(),changeMonthStructure(displayToday2()),changeHeader()'><option value=''>Lng:</option></select></div>";
    ...
    //var tableConstructor = "<tr id='toprow'><td>ሰኞ</td><td>ማክሰኞ</td><td>ረቡዕ</td><td>ሐሙስ</td><td>አርብ</td><td>ቅዳሜ</td><td>እሁድ</td></tr>";
    var hold = 1;
    for (var i = 1; i <= 5; i++) {
        tableConstructor += "<tr>";
    ...
    // divbutton.innerHTML = "<center><a  style='text-decoration:none;color:black; display:block;padding:1px' onclick='ice_hide()'>Hide</a></center>";
...
function whatHolydayIsTodyay(day) {
    ...
    if (selectedMonth == 0 && day == 1)
        hollyDayName = "&nbsp&nbsp&nbsp የዘመን መለወጫ &nbsp";
    else if (selectedMonth == 0 && day == 17)
        hollyDayName = "&nbsp&nbsp&nbsp የመስቀል በዓል &nbsp";
    else if (selectedMonth == 3 && day == genna)
        hollyDayName = "&nbsp&nbsp&nbsp ገና (የእየሱስ ክርስቶስ ልደት በዓል) &nbsp";
    else if (selectedMonth == 4 && day == 11)
        hollyDayName = "&nbsp&nbsp&nbsp  የጥምቀት በዓል &nbsp";
    else if (selectedMonth == 5 && day == 23)
        hollyDayName = "&nbsp&nbsp&nbsp  የአድዋ ድል በዓል &nbsp";
    else if (selectedMonth == 7 && day == 23)
        hollyDayName = "&nbsp&nbsp&nbsp  አለም አቀፍ የላባደሮች ቀን &nbsp";
    else if (selectedMonth == 7 && day == 27)
        hollyDayName = "&nbsp&nbsp&nbsp  የአርበኞች መታሰቢያ &nbsp";
    else if (selectedMonth == sikletMonth && day == sikletDay)
        hollyDayName = "&nbsp&nbsp&nbsp  ስቅለት &nbsp";
    else if (selectedMonth == fasikaMonth && day == fasikaDay)
        hollyDayName = "&nbsp&nbsp&nbsp  ትንሳኤ &nbsp";
    else if (selectedMonth == 8 && day == 20)
        hollyDayName = "&nbsp&nbsp&nbsp  ደርግ የወደቀበት &nbsp";
    else if (selectedMonth == (mewlidMonth - 1) && day == mewlidDay)
        hollyDayName = "&nbsp&nbsp&nbsp መውሊድ (የነብዩ ሙሀመድ ልደት) &nbsp";
    else if (selectedMonth == (FeturMonth - 1) && day == FeturDay)
        hollyDayName = "&nbsp&nbsp&nbsp  ኢድ-አ�?-�?�ጥር (የጾ�? �??ቺ) &nbsp";
    else if (selectedMonth == (AdhaMonth - 1) && day == AdhaDay)
        hollyDayName = "&nbsp&nbsp&nbsp  ኢድ-አ�?-አድሃ (አረ�?�) &nbsp";
...
function highlightToday(dayValue) {
    ...
    if (currentEtMonth == selectedEtMonth && currentEtYear == selectedEtYear) {
        ...
        document.getElementById(currentEtDayId).style.backgroundColor = "rgb(115, 179, 206)";
    }
}
//******************************************************************************
...
function ice_hide() {
    this.hideme();//hides the calendar
...
function getGCOutput(sentIdFromForm) {
    outputGC = sentIdFromForm;
}
...
function convertEcToGc(day) {
    ...
    document.getElementById("div3").innerHTML = "<table ><tr><td>GC Equivalent: </td><td width='100'>" + ECtoGC_Converter(day, month, year) + "</td><td><a  style='cursor: hand;color:white;padding:1px;' onclick='ice_hide()'>Hide</a></td></tr></table>";
    if(document.getElementById(outputGC)!=null)
        document.getElementById(outputGC).value = ECtoGC_Converter(day, month, year);

//return ECtoGC_Converter(day, month, year)ECtoGC_Converter
}
//*****************************************************************************
...
function getToday() {
    ...
    var currentGcMonth = parseInt(today.getMonth());
    var currentGcYear = parseInt(today.getFullYear());
    document.getElementById("div3").innerHTML = "<table><tr><td>GC Equivalent: </td><td width='100'>" + currentGcDay + "/" + (currentGcMonth + 1) + "/" + currentGcYear + "</td><td><a style='cursor: hand;color:#FF82AB;padding:1px; text-align:right;' onclick='ice_hide()'>Hide</a></td></tr></table>";
    //the following 1 line js code pass the feached current date to EC convertor function and put the returned date value in a
    //string format to convertedDate variable which holds the Ethiopian equivalent of system current GC date.
    var convertedDate = GCtoEC_Converter(currentGcDay, currentGcMonth, currentGcYear);
